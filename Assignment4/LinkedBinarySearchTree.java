...
implements BinarySearchTreeADT<T>
{
	...
	/**
	 * Places the elements of the nodes into an ArrayList using an inOrder traversal
	 */
	public BinaryTreeNode<T> balanceTree() 
	{
		ArrayUnorderedList<T> tempList = new ArrayUnorderedList<T>();
		inOrder(root,tempList);
		ArrayList<T> mine = new ArrayList<T>();
		
		for(int i =0;i<tempList.size();i++)
		{
			mine.add(i, tempList.list[i]);
		}
		
		return balanceTree(mine);
	}
	
	/**
	 * Balances the current tree using binary partition
	 * @param holder
	 * @return
	 */
	private  BinaryTreeNode<T> balanceTree(ArrayList<T> holder)
	{
		if (holder.isEmpty())
	        return null;
	    ...
	    else 
	    {
	        ...
	        BinaryTreeNode<T> node= new BinaryTreeNode<T>(holder.get(middle));
	       
	        node.setLeft(balanceTree(new ArrayList<T>(holder.subList(0, middle))));
	        node.setRight(balanceTree(new ArrayList<T>(holder.subList(middle+1,holder.size()))));
